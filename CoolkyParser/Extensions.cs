...
namespace CoolkyRecipeParser
{
    public static class Extensions
    {
        ...
        public static Task ParallelForEachAsync<T>(this IEnumerable<T> source, Func<T, Task> funcBody, int maxParallelLevel = 8)
        {
            ...
            return Task.WhenAll(Partitioner.Create(source).GetPartitions(maxParallelLevel).AsParallel().Select(p => AwaitPartition(p)));
        }
    }
}